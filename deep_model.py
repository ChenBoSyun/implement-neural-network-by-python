...
class Model():
    ...
    def set_weight(self):
        self.W1 = np.random.normal(0, 0.1, (784,204))
        self.W2 = np.random.normal(0, 0.1, (204,202))
        self.W3 = np.random.normal(0, 0.1, (202,10))
        self.b1 = np.array([0.01 for i in range(204)])
        self.b1 = self.b1.reshape([1,204])
        self.b2 = np.array([0.01 for i in range(202)])
        self.b2 = self.b2.reshape([1,202])
        self.b3 = np.array([0.01 for i in range(10)])
        self.b3 = self.b3.reshape([1,10])
    ...
    def update(self,x,y):
        ...
        de_o = (Y-Y_)
        self.W3 = self.W3 - self.lr * H2.T@de_o
        self.b3 = self.b3 - self.lr * np.ones([1,self.batch_size])@de_o
        de_h3 = de_o@self.W3.T*relu_de(L2)
        self.W2 = self.W2 - self.lr * H1.T@de_h3
        self.b2 = self.b2 - self.lr * np.ones([1,self.batch_size])@de_h3
        de_h2 = de_h3@self.W2.T*relu_de(L1)
